...
class Dunnett:
    def __init__(
        ...
        assert (
            simulation.metadata["design"] == "C+LOO"
        ...
        self.result = pd.DataFrame(
            {"group": self.group * self.n_rep, "y": simulation.exresult}
        )
    ...
    def summary(
        ...
        if self.n_rep > 1:
            self.coef = (
                self.result.groupby("group")
                .mean()
                .loc[[v for v in self.group if v != "all factors"]]
            )
            self.baseline = self.result.groupby("group").mean().loc["all factors"]
            self.pvals = dunnett(
                *[
                    self.result[self.result.group == g].y
                    for g in [v for v in self.group if v != "all factors"]
                ],
                control=self.result[self.result.group == "all factors"].y,
                **test_kwargs,
            ).pvalue
            interpretation = (2 * (self.baseline > self.coef).astype(int) - 1) * (
                self.pvals < alpha
            ).reshape(-1, 1).astype(int)
            interpretation = (
                interpretation.applymap(lambda i: {0: "N.S", 1: "Up.", -1: "Down."}[i])
                if dtype == str
                else interpretation
            )
