...
def effectivity_matrix(arr, bool_encoder=lambda x: x != 0):
    ...
    for nrow, idx in enumerate(
        np.lib.stride_tricks.sliding_window_view(
            np.arange(n_f, 0, -1).cumsum(), window_shape=2
        )
    ):
        _edge = arr[idx[0] : idx[1]]
        diag = bool_encoder(np.diag(_edge))
        mat += [
            np.hstack([np.zeros((_edge.size, (n_f - _edge.size))).astype(bool), diag])
        ]
...
def factor_loading(arr):
    ...
    for nrow, idx in enumerate(
        np.lib.stride_tricks.sliding_window_view(
            np.hstack([[0], np.arange(n_f - 1, 0, -1).cumsum()]), window_shape=2
        )
    ):
        mat += [connection[idx[0] : idx[1]].sum(axis=0).astype(bool)]

    return (np.vstack(mat).sum(axis=0) + 1) * (arr[:n_f] != 0).astype(int)
...
def positive_factor_loading(arr):
    ...
    return (np.vstack(mat).sum(axis=0) + 1) * (arr[:n_f] == 1).astype(int)
