...
def ff_anova_power(
    simulator: AbstractSimulator, mlr: MLR, typ: int = 2, alpha: float = 0.05
):
    """
    performs power analysis (n-th-order interaction terms at n>2 are not supported)
    """
    assert (
        simulator.metadata["design"] == mlr.metadata["design"]
    ), "pass the same simulator that was also used for mlr"
    assert (
        simulator.metadata["n_rep"] == mlr.metadata["n_rep"]
    ), "pass the same simulator that was also used for mlr"
    assert (
        simulator.metadata["kwargs"] == mlr.metadata["kwargs"]
    ), "pass the same simulator that was also used for mlr"
    assert (
        simulator.metadata["design"] == "FF"
    ), "only balanced ANOVA is currently supported"
    ...
    y_ijk = pd.concat(
        [
            pd.DataFrame(y, index=exmatrix.index, columns=["y"])
            .reset_index()
            .groupby("index")
            .mean()
            .loc[indices]
    ...
    if interaction_exists and mlr.metadata["order"] >= 2:
        ...
        y_ij = np.hstack(
            [
                np.array(
                    [
                        y[np.where((exmatrix[xi] == si) & (exmatrix[xj] == sj))].mean()
                        for (xi, xj) in combinations(exmatrix.columns, 2)
                    ]
                ).reshape(-1, 1)
                for (si, sj) in signs
            ]
        ...
        y_i_y_j = np.hstack(
            [
                np.array(
                    [
                        y_i[idxij, cij].sum()
                        for idxij in combinations(np.arange(n_factor), 2)
                    ]
                ).reshape(-1, 1)
                for cij in cols
            ]
    ...
    return df.assign(
        power=lambda d: ncf.sf(
            x=f.isf(alpha, dfn=d.dfn, dfd=d.dfd), dfn=d.dfn, dfd=d.dfd, nc=d.nc
        )
    )
