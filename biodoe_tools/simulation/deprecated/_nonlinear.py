...
from biodoe_tools.preferences.cmap import nonlinear
...
class NonLinear(AbstractSimulator):
    def __init__(
        ...
        self.a = {
            i: a
            for i, a in zip([1, 2, 6, 7, 9], np.random.lognormal(**kwarg_a, size=5))
        }
        np.random.seed(seeds[2])
        self.b = {
            i: b
            for i, b in zip([2, 3, 4, 5, 6, 8], np.random.lognormal(**kwarg_b, size=6))
        }
    ...
    def run(
        ...
        def f(x):
            return x * (np.tanh(x) + 1) / 2

        def g(x):
            return gelu(torch.tensor(x)).item()
        ...
        x5 = f(c[5] * (self.v[5] - self.b[4] * x4 + e[5]))
        ...
        x9 = f(c[9] * (self.v[9] + self.b[8] * x8 + e[9]))
        self.c = c
        self.x = {
    ...
    def plot(self, ax: plt.Axes):
        ...
        ax.fill_between([0, 10], [-2, -2], [-1, -1], color=".7", alpha=0.2)
        ...
        ax.text(5, 0.5, "non-linear transformation", ha="center", va="center")
        ax.text(
            5,
            -1.5,
            "output value" if self.y is None else round(self.y, 4),
            ha="center",
            va="center",
        )
