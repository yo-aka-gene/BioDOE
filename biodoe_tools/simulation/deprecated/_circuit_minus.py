...
from biodoe_tools.preferences.cmap import circuit

from ._abstract import AbstractSimulator


class CircuitMinus(AbstractSimulator):
    def __init__(
        self,
        random_state: int = 0,
        ...
        super().__init__(
            n_factor=9, random_state=random_state, cmap=circuit, name=r"Model $\Psi^-$"
        )
        seeds = np.random.randint(0, 2**32, 3)
        np.random.seed(seeds[0])
        self.v = {
            i + 1: vi for i, vi in enumerate(np.random.lognormal(**kwarg_v, size=9))
        }
        np.random.seed(seeds[1])
        self.a = {
            i: a for i, a in zip([1, 2, 7, 8], np.random.lognormal(**kwarg_a, size=4))
        }
    ...
    def run(
        ...
        x5 = f(c[5] * (self.v[5] - (self.b[35] * x3) + self.b[45] * x4 + e[5]))
        x6 = f(
            c[6]
            * (
                self.v[6]
        ...
        self.c = c
        self.x = {
            i + 1: xi for i, xi in enumerate([x1, x2, x3, x4, x5, x6, x7, x8, x9])
        }
        self.y = f(
            self.a[1] * x1 - (self.a[2] * x2) + self.a[7] * x7 + self.a[8] * x8 + e[0]
        )
        return self.y
    ...
    def plot(self, ax: plt.Axes):
        ...
        ax.annotate("", [8.4, 6.3], [3, 9], arrowprops=ap2(3), **arrconf)
        ax.annotate("", [4.7, 3.9], [3, 9], arrowprops=ap2(3), **arrconf)
        ax.annotate("", [1.6, 6.3], [7, 9], arrowprops=ap(4), **arrconf)
        ax.annotate("", [8.6, 6.6], [7, 9], arrowprops=ap2(4), **arrconf)
        ax.annotate("", [5.3, 3.9], [7, 9], arrowprops=ap2(4), **arrconf)
        ...
        ax.annotate("", [6.3, 3], [5, 3], arrowprops=ap2(7), **arrconf)
        ...
        for n, b in self.b.items():
            ax.text(
                ...
                ""
                if datdot.name[int(str(n)[0]) - 1] in [f"X{int(str(n)[0])}", 0.0]
