...
from biodoe_tools.design import DesignMatrix
from biodoe_tools.preferences.pvalues import asterisk, p_format, sign
...
class MLR:
    def __init__(
        ...
        assert issubclass(
            type(simulation), AbstractSimulator
        ), f"pass subclass of AbstractSimulator, got {simulation}[{type(simulation)}]"
        assert simulation.is_executed, (
            "Simluation is not excecuted yet. "
            "Run simulation.simulate before passing it to MLR.__init__"
        )
        exog = (
            DesignMatrix(simulation.exmatrix.values).interactions(order=order)
            if interactions
            else simulation.exmatrix
        )
        self.result = ols(
            "y ~ " + " + ".join(exog.columns), exog.assign(y=simulation.exresult)
        ).fit()
    ...
    def plot(
        ...
        order: int = np.inf,
        const_color: tuple = plt.cm.gray(0.7),
        ...
        params, pvals = self._overwrite_with_anova(
            show_const=show_const, anova=anova, anova_type=anova_type, regex=regex
        )

        displayed_factors = (
            ["Intercept"] + self.metadata["factor_list"]
            if show_const
            else self.metadata["factor_list"].copy()
        )
        ...
        if regex is not None:
            ...
            def find_regex(arr, key):
                return arr[np.vectorize(lambda element: key in element)(arr)]

            displayed_factors = find_regex(np.array(displayed_factors), regex)

        df = (
            pd.DataFrame(params, columns=["coef"])
            .assign(p=pvals)
            .assign(neglog10=-np.log2(pvals.fillna(np.finfo(float).eps)))
            .loc[displayed_factors, :]
            .reset_index()
        )
        ...
        ax.set_xlim([-1, 1])
        ylim = ax.get_ylim()
        ax.vlines(0, *ylim, color=".2", linewidth=0.5, zorder=-300)
        ax.set_ylim(*ylim)
    ...
    def summary(
        ...
        ret = (
            params.apply(lambda x: 1 if x >= 0 else -1)
            * pvals.apply(lambda p: 0 if p >= alpha else 1)
            * pvals.apply(lambda p: 2 if np.isnan(p) else 1)
        )

        return (
            ret.apply(lambda i: ["N.S.", "Up", "N/A", "Down"][i])
            if dtype == str
            else ret.apply(lambda x: np.nan if np.abs(x) == 2 else dtype(x))
        )
    ...
    def _overwrite_with_anova(
        ...
        if anova:
            ...
            try:
                anova_res = sm.stats.anova_lm(self.result, typ=anova_type)
                ss_e = anova_res.loc["Residual", "sum_sq"]
                pvals = anova_res.loc[:, "PR(>F)"].drop("Residual")

                def eta_sq(df_anova):
                    return df_anova.sum_sq / (df_anova.sum_sq + ss_e)
                ...
                pvals = pd.Series([np.nan] * coef.size, index=coef.index)
                params = pd.Series([0] * coef.size, index=coef.index)
        else:
