...
from biodoe_tools.design import FullFactorial
from biodoe_tools.preferences import kwarg_bootstrap as kwarg_bs
...
class TheoreticalEffects:
    def __init__(
        ...
        for s in seeds:
            simulation.simulate(
                design=FullFactorial, n_rep=1, random_state=s, model_kwargs=model_kwargs
            )
            mat = simulation.exmatrix.values
            y = simulation.exresult
            mu += [y.mean()]
            effects += [
                (mat[:, idx] * y).mean() for idx in np.arange(simulation.n_factor)
            ]
        ...
        term_names = eval(
            f"FullFactorial().get_exmatrix(simulation.n_factor)"
            f"{'.interactions(order)' if interactions else '()'}"
        )

        self.mu = np.array(mu)
        self.result = pd.DataFrame(
            {"Coefficient": effects, "term": term_names.columns.tolist() * n_rep}
        )
    ...
    def summary(self, dtype: type = str, kwarg_bootstrap: dict = kwarg_bs):
        self.metadata = {**self.metadata, "kwarg_bootstrap": kwarg_bootstrap}
        return pd.Series(
            [
                self._identifier(
                    bootstrap(
    ...
    def plot(
        ...
        **kwargs,
        ...
        kwarg_bootstrap = (
            self.metadata["kwarg_bootstrap"]
            if "kwarg_bootstrap" in self.metadata
            else kwarg_bs
        )

        data = self.result
        directionality = self.summary(dtype=int, kwarg_bootstrap=kwarg_bootstrap)
        ...
        ylim = ax.get_ylim()
        ax.vlines(0, *ylim, linewidth=0.5, color=".7", linestyle="--")
        ax.set_ylim(ylim)
