...
from biodoe_tools.preferences.cmap import test9
...
class Test9(AbstractSimulator):
    def __init__(
        ...
        super().__init__(
            ...
            cmap=test9,
            name=["ESM9" if model_id == "" else model_id, f"ESM9-#{model_id}"][
    ...
    def run(
        ...
        for i in idx_list:
            ...
            upstream = (
                ...
                else np.sum([self.b[10 * ii + i] * xs[ii] for ii in xs])
            )
    ...
    def plot(self, ax: plt.Axes):
        ...
        sns.scatterplot(data=dat, x="x", y="y", c=test9, s=300, lw=0, ax=ax)

        def fmt_curve(p1, p2, h):
            return (
        ...
        ax.fill_between(
            [1.5, 10.5], [box_bottom] * 2, [box_top] * 2, color=".7", alpha=0.2
        )
        ...
        for i in range(9):
            ...
            if self.a[i + 1] != 0:
                ...
                ax.scatter(
                    x,
                    box_top + 0.25 if self.a[i + 1] > 0 else box_top,
                    color=test9[i],
                    marker="v" if self.a[i + 1] > 0 else "$-$",
                )
                ax.text(
                    x, box_top + 1, "$a_" + f"{(i + 1)}" + "$", va="center", ha="left"
                )
                ...
                if self.b[10 * (i + 1) + j + 1] != 0:
                    ...
                    if ii == 0:
                        ...
                        ax.scatter(
                            ...
                            y2 + 0.5 if self.b[10 * (i + 1) + j + 1] > 0 else y2 + 0.35,
                            color=test9[i],
                            marker="^" if self.b[10 * (i + 1) + j + 1] > 0 else "$|$",
                            alpha=alpha,
                        )
                        ax.text(
                            (x + x2) / 2,
                        ...
                        ax.scatter(
                            x2 - 0.15 if self.b[10 * (i + 1) + j + 1] > 0 else x2 - 0.1,
                            y2 + 0.86 if self.b[10 * (i + 1) + j + 1] > 0 else y2 + 0.7,
                            color=test9[i],
                            marker="<" if self.b[10 * (i + 1) + j + 1] > 0 else "$-$",
                            alpha=alpha,
                        )
                        ax.text(
                            *get_top((x, y), (x2, y2), h, True),
                            "$b_{" + f"{(i + 1)}{(j + 1)}" + "}$",
                            va="bottom",
                            ha="center",
                        )
