...
def beta(X_des: np.ndarray, y_obs: np.ndarray) -> np.ndarray:
    """
    estimates a coefficient vector from given design matrix X_des
    and observation vector y_obs
    """
...
def lambda_i(simulator: AbstractSimulator, i: int) -> float:
    ...
    return (
        norm2(
            Qr(simulator, i)
            @ Xi(simulator, i)
            * beta(X(simulator), Y(simulator))[i + 1].item()
        ).item()
        / sigma2(simulator).item()
    )


def Fi(simulator: AbstractSimulator, i: int) -> float:
    return (SSi(simulator, i) / phi_i(simulator, i)) / sigma2(simulator)


def power(simulator: AbstractSimulator, i: int, alpha: float = 0.05) -> float:
    """
    calculate $1-\beta$ for the given factor
    """
    return ncf.sf(
        x=f.isf(q=alpha, dfn=phi_i(simulator, i), dfd=phi_e(simulator)),
        dfn=phi_i(simulator, i),
        dfd=phi_e(simulator),
