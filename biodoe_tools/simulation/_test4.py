...
from biodoe_tools.preferences.cmap import test4
...
class Test4(AbstractSimulator):
    def __init__(
        ...
        super().__init__(
            ...
            cmap=test4,
            name=["ESM4" if model_id == "" else model_id, f"ESM4-#{model_id}"][
        ...
        self.v = {
            i + 1: vi
            for i, vi in enumerate(np.random.lognormal(**kwarg_v, size=self.n_factor))
        }
    ...
    def run(
        ...
        self.y = f(
            sum([self.a[i + 1] * self.x[i + 1] for i in range(self.n_factor)]) + e[0]
        )
        return self.y
    ...
    def plot(self, ax: plt.Axes):
        ...
        edge_pos = {
            12: (
                [datdot.h[1] - 0.5 / np.sqrt(2), datdot.v[1] + 0.5 / np.sqrt(2)],
                [datdot.h[0], datdot.v[0]],
            ),
            13: ([datdot.h[2], datdot.v[2] + 0.5], [datdot.h[0], datdot.v[0]]),
            14: ([datdot.h[3], datdot.v[3] + 0.75], [datdot.h[0], datdot.v[0]]),
            23: (
                [datdot.h[2] - 0.5 / np.sqrt(2), datdot.v[2] + 0.5 / np.sqrt(2)],
                [datdot.h[1], datdot.v[1]],
            ),
            24: ([datdot.h[3] - 0.5, datdot.v[3]], [datdot.h[1], datdot.v[1]]),
            34: (
                [datdot.h[3] - 0.5 / np.sqrt(2), datdot.v[3] + 0.5 / np.sqrt(2)],
        ...
        a_aps = {k: ap(k) if a >= 0 else ap2(k) for k, a in self.a.items()}
        ...
        for idx, a in self.a.items():
            if a != 0:
                ax.annotate(
                    "",
                    [datdot.h[idx - 1], 1],
                    [datdot.h[idx - 1], datdot.v[idx - 1]],
                    arrowprops=a_aps[idx],
                    **arrconf,
                )
                ax.text(
                    datdot.h[idx - 1],
