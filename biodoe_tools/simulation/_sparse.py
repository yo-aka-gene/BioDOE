...
from biodoe_tools.preferences.cmap import sparse
...
class Sparse(AbstractSimulator):
    def __init__(
        ...
        super().__init__(
            n_factor=9, random_state=random_state, cmap=sparse, name=r"Model $\Lambda$"
        )
        ...
        self.a = {i: a for i, a in zip([1, 2], np.random.lognormal(**kwarg_a, size=2))}
        np.random.seed(seeds[2])
        self.b = {
            i: b
            for i, b in zip(
                [45, 46, 47, 57, 67, 78, 79], np.random.lognormal(**kwarg_b, size=7)
            )
        }
    ...
    def run(
        ...
        x7 = f(
            c[7]
            * (self.v[7] + self.b[47] * x4 - (self.b[57] * x5) + self.b[67] * x6 + e[7])
        )
        ...
        self.y = f(self.a[1] * x1 - (self.a[2] * x2) + e[0])
        return self.y
