...
from biodoe_tools.preferences import subplots


class AbstractSimulator:
    def __init__(self, n_factor: int, random_state: int, cmap: list, name: str):
    ...
    def run(self, design_array: np.ndarray, random_state: int = 0):
        self.c = {i + 1: (ci + 1) / 2 for i, ci in enumerate(design_array)}
        np.random.seed(random_state)
    ...
    def simulate(
        ...
        **kwargs,
        ...
        self.metadata = {
            ...
            "n_rep": n_rep,
            "factor_list": self.design.get_exmatrix(
                self.n_factor, **kwargs
            )().columns.tolist(),
            "kwargs": kwargs,
        }
        self.exmatrix = pd.concat(
            [self.design.get_exmatrix(self.n_factor, **kwargs)()] * n_rep
        )
        np.random.seed(random_state)
    ...
    def scatterview(
        ...
        assert self.is_executed, (
            ...
            "Run self.simulate before calling self.scatterview"
        )
        ax = subplots(self.n_factor)[1] if ax is None else ax
        factors = [f"X{i + 1}" for i in range(self.n_factor)]
        n_trials = len(
            self.design.get_exmatrix(self.n_factor, **self.metadata["kwargs"])()
        )
        ...
        for i, a in enumerate(ax.ravel()):
            ...
            df = pd.DataFrame(
                {
                    factor: [
                        ["$-$", "$+$"][v]
                        for v in ((1 + self.exmatrix.loc[:, factor]) / 2).astype(int)
                    ],
            ...
            def get(val):
                return self.exresult[np.where(self.exmatrix.loc[:, factor] == val)]
            ...
            a.set(xlabel="output values", ylabel="", title=factor)
