...
from biodoe_tools.design import DOE
from biodoe_tools.preferences import textcolor
from biodoe_tools.preferences.cmap import prototype
...
class Prototype(AbstractSimulator):
    def __init__(
        ...
        random_state: int = 0,
        model_id: int = "",
        kwarg_v: dict = dict(mean=1, sigma=0.8),
        kwarg_a: dict = dict(mean=2, sigma=0.3),
        kwarg_b: dict = dict(mean=1, sigma=0.5),
    ):
        super().__init__(
            n_factor=5,
        ...
        self.v = {
            i + 1: vi for i, vi in enumerate(np.random.lognormal(**kwarg_v, size=5))
        }
        np.random.seed(seeds[1])
        self.a = {
            i: a
            for i, a in zip([1, 2, 3, 4, 5], np.random.lognormal(**kwarg_a, size=5))
        }
    ...
    def run(
        ...
        x4 = f(
            c[4]
            * (self.v[4] + self.b[14] * x1 + self.b[24] * x2 + self.b[34] * x3 + e[4])
        )
        x5 = f(
            c[5]
            * (
    ...
    def plot(self, ax: plt.Axes):
        ax.fill_between([0, 10], [0, 0], [1, 1], color=".7", alpha=0.2)

        datdot = pd.DataFrame(
            {
                "name": [
                    f"X{i + 1}" if self.x is None else round(self.x[i + 1], 2)
                    for i in range(5)
                ],
                "h": [1, 3, 5, 7, 9],
                "v": [7, 6, 5, 4, 3],
                "c": [
                    ".7" if self.x is not None and self.x[i + 1] == 0 else self.cmap[i]
                    for i in range(5)
                ],
                "alpha": [
                    0.2 if self.x is not None and self.x[i + 1] == 0 else 1
                    for i in range(5)
                ],
            }
        )
        ...
        def ap(n) -> dict:
            return dict(
                ...
                facecolor=datdot.c[n - 1],
                edgecolor=datdot.c[n - 1],
        ...
        def angleA(n):
            return [0, -90, 0, -45][n - 1]

        def angleB(n):
            return [90, 180, 105, 120][n - 1]
        ...
        edge_pos = {
            12: (
                [datdot.h[1] - 1 / np.sqrt(5), datdot.v[1] + 0.5 / np.sqrt(5)],
                [datdot.h[0], datdot.v[0]],
            ),
            13: ([datdot.h[2], datdot.v[2] + 0.5], [datdot.h[0], datdot.v[0]]),
            14: ([datdot.h[3], datdot.v[3] + 0.5], [datdot.h[0], datdot.v[0]]),
            15: ([datdot.h[4], datdot.v[4] + 0.5], [datdot.h[0], datdot.v[0]]),
            23: (
                [datdot.h[2] - 1 / np.sqrt(5), datdot.v[2] + 0.5 / np.sqrt(5)],
                [datdot.h[1], datdot.v[1]],
            ),
            24: ([datdot.h[3] - 0.5, datdot.v[3]], [datdot.h[1], datdot.v[1]]),
            25: ([datdot.h[4] - 0.5, datdot.v[4]], [datdot.h[1], datdot.v[1]]),
            34: (
                [datdot.h[3] - 1 / np.sqrt(5), datdot.v[3] + 0.5 / np.sqrt(5)],
                [datdot.h[2], datdot.v[2]],
            ),
            35: (
                [datdot.h[4] - 0.25, datdot.v[4] + np.sqrt(3) / 4],
                [datdot.h[2], datdot.v[2]],
            ),
            45: (
                [datdot.h[4] - np.sqrt(3) / 4, datdot.v[4] + 0.25],
                [datdot.h[3], datdot.v[3]],
            ),
        }
        b_pos = {
        ...
        for idx, b in self.b.items():
            if b != 0:
                ...
                ax.text(
                    *b_pos[idx],
                    ""
                    if datdot.name[int(str(idx)[0]) - 1]
                    in [f"X{int(str(idx)[0])}", 0.0]
                    else r"$\times$" + f"{b.round(2)}",
                    ha="center",
                    va="center",
                    size=6,
                )
        ...
        for n, a in self.a.items():
            ax.text(
                ...
                1.75,
                ""
                if datdot.name[n - 1] in [f"X{n}", 0.0]
                else r"$\times$" + f"{a.round(2)}",
                ha="center",
                va="center",
                size=6,
            )
        ...
        if self.c is not None:
            for n, c in self.c.items():
                ax.scatter(
                    *{i + 1: (h, v) for i, h, v in zip(range(9), datdot.h, datdot.v)}[
                        n
                    ],
                    marker="x",
                    s=300,
        ...
        ax.text(
            5,
            0.5,
            "output value" if self.y is None else round(self.y, 2),
            ha="center",
            va="center",
        )
    ...
    def scatterview(self, ax: plt.Axes):
