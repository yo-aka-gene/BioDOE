...
def aptitude_score(res, gt):
    def f(res, gt):
        return (
            accuracy_score(res, gt)
            if gt.unique().size == 1
            else cohen_kappa_score(res, gt, weights="linear")
        )
...
class MetricConfigurator:
    ...
    def __call__(self) -> dict:
        return {"f": self.f, "name": self.name}
...
def simulate(
    ...
    simulator.simulate(
        design=design, n_rep=n_rep, random_state=random_state, model_kwargs=model_kwargs
    )
    return simulator


def mlr_anova(simulator: AbstractSimulator) -> pd.Series:
    warnings.simplefilter("ignore")
    return MLR(simulator).summary(anova=True, dtype=int)


def theoretical_effect(
    simulation: AbstractSimulator, random_state: int, model_kwargs: dict
) -> TheoreticalEffects:
    return TheoreticalEffects(
        simulation=simulation, random_state=random_state, model_kwargs=model_kwargs
    )
...
def power_table_formatter(
    ...
    return power_func(simulator).assign(
        noise=pd.Series(
            [
                noise_names[idx // (n_rep * n_range.size)]
                for _ in simulator.metadata["factor_list"]
            ]
        )
    )
...
class Benchmarker:
    def __init__(
        ...
        self.metric = MetricConfigurator(
            evaluation_metric=evaluation_metric, metric_name=metric_name
        )
        ...
        caller = (
            ...
            else (lambda m: m(edge_assignment=edge_assignment, model_id=model_id))
        )
        self.conditions = {
            k: list(
        ...
        self.theoretical = Parallel(n_jobs=n_jobs)(
            delayed(theoretical_effect)(
                ...
                if edge_assignment is None
                else simulator(edge_assignment=edge_assignment),
                random_state=random_state,
                model_kwargs=nc,
            )
        ...
        self.ground_truth = Parallel(n_jobs=n_jobs)(
            delayed(lambda te: te.summary(dtype=int))(self.theoretical[i])
            for i in np.tile(np.arange(self.noise.size), n_range.size * seeds.size)
            .reshape(-1, self.noise.size)
            .T.ravel()
        )
        ...
        self.results = {
            k: Parallel(n_jobs=n_jobs)(delayed(mlr_anova)(m) for m in cond)
            for k, cond in tqdm(
                self.conditions.items(),
                total=len(self.conditions),
            ...
            self.results = {
                **self.results,
                "pair": Parallel(n_jobs=n_jobs)(
                    delayed(lambda m: Dunnett(m).summary(dtype=int))(m)
                    for m in self.conditions["cloo"]
                ),
            }
        ...
        self.scores = pd.DataFrame(
            {
                ...
                "err": np.ravel(
                    [[v] * n_range.size * seeds.size for v in self.noise.arr]
                ),
                **{
                    f"{k}_metric": [
                        self.metric.f(res, gt)
                        for res, gt in tqdm(
                            zip(results, self.ground_truth),
        ...
        def power(key):
            return anova_power if key != "pair" else dunnett_power

        def condition_key(key):
            return "cloo" if key == "pair" else key
    ...
    def plot_benchmarking(
        ...
        items = [
            item
            for item in items
            if (item != "pair") or (self.metadata.dunnett and show_dunnet)
        ]
        ...
        if noise is None:
            ...
            for i, a in enumerate(ax.ravel()):
                ...
                for k in items:
                    sns.lineplot(
                        data=self.scores[self.scores.err == e],
                        x="n",
                        y=f"{k}_metric",
                        marker="s",
                        ax=a,
                        label=self.conditions[k][0].metadata["design"]
                        if k != "pair"
                        else "Dunnett",
                        color=cmap[k],
                        markeredgewidth=0,
                        errorbar=("ci", 95),
                        n_boot=1000,
                        seed=self.metadata.random_state,
                    )
                a.set_ylim(-0.05, 1.05)
                a.set_xticks(self.metadata.n_range.tolist())
                a.set_yticks(np.linspace(0, 1, 6).tolist())
                a.set(
                    title=self.noise.names[i],
                    xlabel="N: Num. of replication",
            ...
            for k in items:
                sns.lineplot(
                    data=self.scores[self.scores.err == noise],
                    x="n",
                    y=f"{k}_metric",
                    marker="s",
                    ax=ax,
    ...
    def plot_power(
        ...
        fig, ax = plt.subplots(
            len(items),
            self.noise.size,
            figsize=(3 * self.noise.size, 2 * len(items)),
            sharex=True,
            sharey=True,
        )
        plt.subplots_adjust(wspace=wspace, hspace=hspace)

        bbox_to_anchor = (1, 0.5) if self.metadata.dunnett else (1, 1)
        center = np.floor(len(items) / 2).astype(int)

        for i, a in enumerate(ax.ravel()):
            sns.lineplot(
                data=self.power[
                    (self.power.noise == self.noise.names[i % self.noise.size])
                    & (self.power.model == items[i // self.noise.size])
                ].reset_index(),
                x="n_rep",
                y="power",
                hue="term",
            ...
            a.set_title(
                self.noise.names[i % self.noise.size]
            ) if i // self.noise.size == 0 else a.set_xlabel("N: Num. of replication")
            a.legend(
                loc="center left",
...
class BenchmarkingPipeline:
    def __init__(
        ...
        self.configuration = {
            k: Benchmarker(**d, suffix=fmt_suffix(k))
            for k, d in tqdm(
                config.items(),
    ...
    def plot(
        self,
        func: list = ["plot_groundtruth", "plot_benchmarking", "plot_power"],
        kwargs: dict = {
            "plot_groundtruth": {},
            "plot_benchmarking": {},
            "plot_power": {},
        ...
        for f in tqdm(func, total=len(func), desc="Plotting"):
            for k, bm in tqdm(
                self.configuration.items(), total=len(self.configuration), desc=f
            ):
                fig, ax = eval(f"bm.{f}")(**kwargs[f])
                if savefig:
                    fig.savefig(
                        f"{where}/{titles[f]}{bm.suffix}{extension}", **kwarg_save
                    )
...
class DOptimizationBenchmarker(Benchmarker):
    def __init__(
        ...
        caller = (
            (lambda m: m())
            if edge_assignment is None
            else (lambda m: m(edge_assignment=edge_assignment))
        )
        self.conditions = {
        ...
        self.scores = pd.DataFrame(
            {
                "err": np.ravel(
                    [[v] * n_add.size * seeds.size for v in self.noise.arr]
                ),
                **{
                    f"{k}_nmax": Parallel(n_jobs=n_jobs)(
                        delayed(lambda simulator: len(simulator.exmatrix))(m)
                        for m in cond
                    )
                    for k, cond in tqdm(
                        self.conditions.items(),
                ...
                **{
                    f"{k}_metric": Parallel(n_jobs=n_jobs)(
                        delayed(self.metric.f)(res, gt)
                        for res, gt in zip(results, self.ground_truth)
                    )
                    for k, results in tqdm(
                        self.results.items(),
                        total=len(self.results),
                ...
                **{
                    f"{k}_d": np.array(
                        Parallel(n_jobs=n_jobs)(
                            delayed(
                                lambda simulator: d_criterion(
                                    sm.add_constant(simulator.exmatrix)
                                )
                            )(m)
                            for m in cond
                        )
        ...
        self.scores_without_doptimization = pd.concat(
            [
                from_benchmarker.scores[from_benchmarker.scores.n == N].reset_index(
                    drop=True
                ),
                pd.DataFrame(
                    {
                        f"{k}_d": d_criterion(
                            sm.add_constant(
                                np.vstack(
                                    [des().get_exmatrix(n_factor=n_factor)().values] * N
                                )
                            )
                        )
                        * np.ones(self.noise.size * self.metadata.n_rep)
                        for k, des in self.metadata.designs.items()
                    }
                ),
                pd.DataFrame(
                    {
                        f"{k}_nmax": len(des().get_exmatrix(n_factor=n_factor)())
                        * N
                        * np.ones(self.noise.size * self.metadata.n_rep).astype(int)
                        for k, des in self.metadata.designs.items()
                    }
                ),
            ],
                ...
                pd.concat(
                    Parallel(n_jobs=n_jobs)(
                        delayed(power_pddf_formatter)(
                            idx=i,
                            simulator=m,
                            d=self.scores[f"{k}_d"].values,
                            nmax=self.scores[f"{k}_nmax"].values,
                            n_factor=n_factor,
                            noise_names=self.noise.names,
                            n_rep=self.metadata.n_rep,
                            n_add=n_add,
                        )
    ...
    def plot_benchmarking(
        ...
        if noise is None:
            ...
            for i, a in enumerate(ax.ravel()):
                ...
                for k in items:
                    df_group = (
                        self.scores[self.scores.err == e].groupby(f"{k}_nmax").mean()
                    )
                    xlist = (
                        list(df_group[f"{k}_d"])
                        + df[df.err == e]
                        .mean()[[f"{k2}_d" for k2 in self.metadata.designs]]
                        .tolist()
                    )
                    ylist = (
                        list(df_group[f"{k}_metric"])
                        + (
                            df[df.err == e].mean()[
                                [f"{k2}_metric" for k2 in self.metadata.designs]
                            ]
                        ).tolist()
                    )
                    tlist = (
                        ...
                        + (
                            df[df.err == e]
                            .mean()
                            .astype(int)[[f"{k2}_nmax" for k2 in self.metadata.designs]]
                        ).tolist()
                    )
                    label_loc = -0.1 if min(ylist) >= 0.2 else 0.1
            ...
            for k in items:
                df_group = (
                    self.scores[self.scores.err == noise].groupby(f"{k}_nmax").mean()
                )
                xlist = (
                    list(df_group[f"{k}_d"])
                    + df[df.err == noise]
    ...
    def plot_power(
        ...
        bbox_to_anchor = (1, 0.5) if len(items) % 2 == 1 else (1, 1)
        center = np.floor(len(items) / 2).astype(int)
        ...
        for i, a in enumerate(ax.ravel()):
            sns.lineplot(
                data=self.power[
                    (self.power.noise == self.noise.names[i % self.noise.size])
                    & (self.power.model == labels[items[i // self.noise.size]])
                ].reset_index(),
                x="nmax",
                y="power",
                hue="term",
...
class DOptimizationBenchmarkingPipeline(BenchmarkingPipeline):
    def __init__(
        ...
        self.configuration = {
            ...
            for k, bm in tqdm(
                from_pipeline.configuration.items(),
                total=len(from_pipeline.configuration),
