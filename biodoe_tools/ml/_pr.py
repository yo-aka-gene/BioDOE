...
def plot_pr(model, x, y, ax, cmap, label_dict):
    assert len(ax.ravel()) >= 4, f"At least 4 subplots are required; got {ax}"
    ...
    for (i, x_), y_, ap, label, base in zip(
        ...
        ax[i].plot(
            [0, 1], [base, base], c="gray", label="baseline", zorder=1, alpha=0.5
        )
    ...
    [
        sp.plot([0, 1], [1, 1], linestyle=(0, (1, 2)), c=".2", label="ideal", zorder=2)
        for sp in ax
    ]
