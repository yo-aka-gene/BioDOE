...
def design_heatmap(
    ...
    **kwargs,
    ...
    assert isinstance(annot_kws, dict), f"pass dict, got {annot_kws}[{type(annot_kws)}]"
    assert (
        "+" in annot_kws and "-" in annot_kws
    ...
    sns.heatmap(
        dsm,
        annot=dsm.applymap(lambda x: annot_kws["+"] if x else annot_kws["-"]),
        fmt="s",
        cbar=False,
        ax=ax,
        **kwargs,
