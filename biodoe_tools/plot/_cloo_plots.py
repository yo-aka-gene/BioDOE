...
from biodoe_tools.preferences.pvalues import asterisk, p_format, select, sign
from biodoe_tools.simulation import AbstractSimulator


def legacy_scatterview(
    simulation: AbstractSimulator,
    cmap: list = None,
    const_color: tuple = plt.cm.gray(0.2),
    ax: plt.Axes = None,
):
    assert issubclass(
    ...
    assert simulation.is_executed, (
        ...
        "Run simulation.simulate before passing it to legacy_scatterview"
    )
    assert simulation.metadata["design"] == "C+LOO", (
        f"Only simulation models based on the C+LOO design is supported; "
        f"got {simulation.metadata['design']}"
    )
    ...
    sns.scatterplot(
        ...
        * simulation.metadata["n_rep"],
        x=simulation.exresult,
        s=30,
        color=([const_color] + cmap) * simulation.metadata["n_rep"],
    )

    counterpart = [None] + [
        np.mean([v for loc, v in enumerate(simulation.exresult) if loc % nrows != i])
        for i in range(simulation.n_factor)
    ]
    ...
    ax.set(
        ...
        ylabel="",
        title=(
            f"{simulation.metadata['design']} design "
            f"(N={simulation.metadata['n_rep']})",
        ),
    )
...
def bio_scatterview(
    ...
    ylim = ax.get_ylim()
    ax.vlines(
        simulation.exresult.reshape(3, -1).mean(axis=0)[0],
        *ylim,
        color=".2",
        linewidth=1,
    )
...
def bio_multicomp(
    ...
    df = pd.DataFrame(
        {
            "group": ["all factors" if i == 0 else f"X{i} KD" for i in range(nrows)]
            * simulation.metadata["n_rep"],
    ...
    for i, m, sd, c in zip(range(nrows), means, sds, [const_color] + cmap):
        ax.vlines(
            m - sd, i - (yrange * 0.03), i + (yrange * 0.03), color=c, linewidth=1.5
        )
        ax.vlines(
            m + sd, i - (yrange * 0.03), i + (yrange * 0.03), color=c, linewidth=1.5
        )
        ax.vlines(m, i - (yrange * 0.02), i + (yrange * 0.02), color=c, linewidth=1.5)
    ...
    ax.set(
        ...
        ylabel="",
        title=(
            f"{simulation.name} analyzed with "
            f"{simulation.metadata['design']} design "
            f"(N={simulation.metadata['n_rep']})"
        ),
    )
